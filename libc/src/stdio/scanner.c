...
static void parse_specifier(struct scanner_state *state)
{
	...
	state->spec_size = 1;
	switch (*state->format) {
		case '\0': state->mode = MODE_DONE; return;
		case 'i':
		case 'u': state->mode = MODE_INTEGER; break;
		case 'd': state->mode = MODE_DEC; break;
		case 'o': state->mode = MODE_OCT; break;
		case 'x': state->mode = MODE_HEX; break;
...
static void mode_oct(struct scanner_state *state)
{
}

static void mode_hex(struct scanner_state *state)
{
}
...
static void mode_next(struct scanner_state *state)
{
	state->current = 0;
}
